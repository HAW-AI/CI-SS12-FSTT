...
import static haw.ai.ci.node.BinOpNode.BinOp.HIEQ_OP;
import static haw.ai.ci.node.BinOpNode.BinOp.HI_OP;
import static haw.ai.ci.node.BinOpNode.BinOp.LOEQ_OP;
...
import haw.ai.ci.node.SelectorNode;
		...
		if (argv.length == 0) {
			...
			for (int i = 0; i < argv.length; i++) {
				...
				try {
					...
					PrintWriter w = new PrintWriter("codetxt");
					w.print(abstractTree.code());
					w.close();
